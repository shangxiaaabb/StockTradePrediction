...
#        security_df = get_all_securities(types = ['options'], date = self.date).reset_index()
...
#        codes = security_df['code'].tolist()
        ...
        security_df.to_csv(f'{self.data_dir}/security_file/{self.date_str}_options.csv.gz', compression='gzip',index=False)
#        close_df = get_price(ukeys, start_date = self.date, end_date = self.date, fields = ['open', 'close', 'high', 'low', 'volume', 'money', 'high_limit', 'low_limit', 'factor'])
        ...
        for index, row in security_df.iterrows():
            print('downloading options ' + row['code'])
            code = row['code']
            close_df = get_price(code, start_date=row['list_date'], end_date=self.date, frequency='daily', fields=['open', 'close', 'high', 'low', 'volume', 'money', 'avg', 'high_limit', 'low_limit', 'pre_close', 'paused', 'factor', 'price', 'open_interest'], skip_paused=False, count=None)
            close_df['code'] = code
...
def main():
    ...
    for date in dates:
        ...
        try:
            JQDataLoader(date, '/volume/sinoalgo/Options').run()
