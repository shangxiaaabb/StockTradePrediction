...
logger = logger_init('train', conf.scalar)
...
def train(train_loader, model, criterion, epoch, optimizer):
    ...
    for batch_i, (input_data, output_data) in enumerate(train_loader):
        ...
        batch_lr = decay_lr_poly(conf.lr, epoch, 0, conf.epochs, batches, conf.warm_up, conf.power)
        ...
        if (batch_i+ 1) % conf.print_freq == 0:
            logger.info(
                'EPOCH [{}/{}], BATCH [{}/{}], DataTime [{:.3f}], BatchTime [{:.3f}] LOSS [{:.6f}]'.format(
        ...
        logger.info('EPOCH[{}/{}] LOSS[{:.6f}]'.format(
            epoch + 1, conf.epochs, loss.get_avg()))
        logger.info('-' * 200)

        # visualize train (epoch)
        writer.add_scalar('train-epoch/Loss', loss.get_avg(), epoch + 1)
...
def main(input_path, output_path):
    ...
    model = GAT(n_feat= 9, n_hid= 18, n_class= 3)
    ...
    for epoch in range(conf.epochs):
        ...
        train(train_loader= train_dataloader, model= model, criterion= criterion, optimizer= optimizer)
