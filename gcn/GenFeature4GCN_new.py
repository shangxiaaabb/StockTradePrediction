...
class GenF4GCN():
    ...
    def df2matrix(file,col_name):
        ...
        first_bin = df_org[df_org['bin_num'].isin([0])].groupby('date')['bin_volume'].sum()
        # 将结果映射到原始数据框中的date，并使用fillna(0)填充缺失值
        df_org['first_bin'] = df_org['date'].map(first_bin)
    ...
    def genNewFeature4BinVolume(stock_info,file,lag_day=3,lag_bin=3,lag_week = 1):
        ...
        f8 = GenF4GCN.df2matrix(file,'first_bin')
        ...
        for i in range(len(mdata.index)):
            for j in range(len(mdata.columns)):
                ...
                f_all = [f1.iloc[i,j],f2.iloc[i,j],f3.iloc[i, j],f4.iloc[i,j],f5.iloc[i,j],f6.iloc[i,j],f7.iloc[i,j],f8.iloc[i,j]]   

                f_all = [0 if np.isnan(i) else i for i in f_all]
                result.iloc[i, j] = [float('{:.4f}'.format(i)) for i in f_all]      

        result =  result.drop('bin0', axis=1)
